...
- to allow for "vertical concatenation" of the design matrices of each runs
  regressors must be ordered in the same way and dummy regressors must be added
  when a given regressor does not exist
- one current limitation for the scrubbing regressors generated by fmriprep is
  that even though they have the same "name" (for example motion-outlier01) they
  may actually censor a different time point are thus not equivalent even though
  they will be vertically concatenated.
...
<img src="./images/mod_1_001.png" alt="drawing" height="500"/>
